...
class TestCoverageGate(unittest.TestCase):
    ...
    def parse_coverage_output(self, stdout: str, stderr: str) -> Tuple[float, Dict[str, float], List[str]]:
                ...
                if len(parts) >= 4:
                    ...
                    if coverage_str.endswith('%'):
                        try:
                            ...
                            if any(critical.replace('\\', '/') in filename_normalized for critical in self.critical_modules):
        ...
        if overall_coverage == 0.0 and stderr:
            if "No data to report" in stderr:
                raise unittest.SkipTest("No coverage data available. Run 'coverage run -m pytest' first.")
            else:
    ...
    def test_overall_coverage_meets_minimum(self):
        ...
        self.assertGreaterEqual(
            ...
            "\n".join(f"  - {file}" for file in uncovered_files) if uncovered_files else ""
        )
    ...
    def test_critical_modules_coverage(self):
        ...
        try:
            ...
            critical_overall, critical_files, critical_uncovered = self.parse_coverage_output(result.stdout, result.stderr)
        ...
        self.assertGreaterEqual(
            ...
            "\n".join(f"  - {file}" for file in critical_uncovered) if critical_uncovered else ""
        )
    ...
    def test_coverage_report_format(self):
        ...
        self.assertGreater(len(file_coverage), 0, "Coverage report should include individual file coverage")
