...
class TestWebSocketClient:
    ...
    async def test_basic_connection(self, ws_config, mock_websocket):
        ...
        with patch.object(client, '_create_connection', side_effect=mock_connect):
            try:
    ...
    async def test_disconnect(self, ws_config, mock_websocket):
        ...
        with patch.object(client, '_create_connection', return_value=mock_websocket):
            await asyncio.wait_for(client.connect(), timeout=2.0)
    ...
    async def test_send_message(self, ws_config, mock_websocket):
        ...
        with patch.object(client, '_create_connection', return_value=mock_websocket):
            try:
                await asyncio.wait_for(client.send("test_method", {"param": "value"}), timeout=2.0)
        ...
        with patch.object(client, '_create_connection', side_effect=mock_connect):
            with patch('asyncio.sleep') as mock_sleep:
                try:
                    ...
                    await asyncio.wait_for(client._handle_connection_error(ConnectionError("Test")), timeout=5.0)
...
class TestStreamingClient:
    ...
    def streaming_client(self, mock_http_client):
        ...
        with patch('accumulate_client.client.streaming.WebSocketClient') as mock_ws_class:
            # Create an AsyncMock for the WebSocket client
            mock_ws = AsyncMock()
            mock_ws.connect = AsyncMock()
            mock_ws.disconnect = AsyncMock()
            mock_ws.subscribe = AsyncMock()
            mock_ws.add_event_hook = Mock()  # This one is not async
            mock_ws_class.return_value = mock_ws

            client = StreamingAccumulateClient(mock_http_client)
            client.ws_client = mock_ws  # Explicitly set the mock
            return client
