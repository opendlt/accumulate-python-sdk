...
class TestWebSocketClient:
    ...
    async def test_disconnect(self, ws_config, mock_websocket):
        ...
        # Create real task objects that can be properly cancelled and awaited
        async def dummy_reader():
            await asyncio.sleep(0)
        async def dummy_ping():
            await asyncio.sleep(0)

        mock_reader_task = asyncio.create_task(dummy_reader())
        mock_ping_task = asyncio.create_task(dummy_ping())
