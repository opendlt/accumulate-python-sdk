...
def _create_mock_client(endpoint: str):
    ...
    class MockClient:
        ...
        def query(self, url: str) -> Dict[str, Any]:
            """Mock query response."""
            if "/ACME" in url or "/tokens" in url:
                # Token account query - check if we have a custom balance
                balance = self._balances.get(url, 100_000_000 if "/ACME" in url else 0)  # Default 1 ACME for lite accounts, 0 for ADI token accounts
                return {
                    "data": {
                        "balance": balance,
