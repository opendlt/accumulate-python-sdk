...
def main():
    ...
    if existing_adi:
        ...
        if key_page_data:
            ...
            if credits > 1000000:  # > 1 credit
                print("✅ ADI exists with sufficient credits")
                return
    ...
    tassert(adi_data, "ADI creation failed")

    # Check key page credits
    key_page_data = query_with_retry(client, key_page_url)
    tassert(key_page_data, "Key page creation failed")

    credits = key_page_data.get('creditBalance', 0)

    # Check remaining lite balance
    ...
    print_result("Final Status", {
        "ADI URL": adi_url,
        "ADI Status": "Created" if adi_data else "Failed",
        "Key Page URL": key_page_url,
        "Key Page Credits": format_credits(credits),
        "Lite Balance Remaining": format_tokens(final_lite_balance) + " ACME"
    })

    # Verify sufficient credits
    tassert(credits > 1000000, f"Insufficient credits: {format_credits(credits)}")

    print("\nSUCCESS: ADI created with sufficient credits!")
    print(f"ADI: {adi_url}")
    print(f"Credits: {format_credits(credits)}")
    print("\nNext: Run 03_token_account_and_transfer.py to create token accounts")
